...
var make_parse = function () {
	...
	var parse = function (rbp) {
		...
		while (left) {
			if (symbol_table[token.id].lef_typ)
				lbp = symbol_table[token.id].lef_lbp;
			else if (symbol_table[token.id].nul_typ && implicit)
				lbp = MUL_BP;
			else
				lbp = MAX_BP + 1;
			// parse like Agda here: https://softwareengineering.stackexchange.com/questions/310735/language-with-two-binary-operators-of-same-precedence-left-associative-right
			// same precedence: associate to the right-associative operator
			if (!((rbp > lbp) || (rbp === lbp && symbol_table[token.id].lef_typ && symbol_table[token.id].lef_lbp < symbol_table[token.id].lef_rbp))) // associate to the right (>=) or to the left (>) by default at same precedence
				break;
	...
	var lef_type = function (left) {
		if (token && token.id && symbol_table[token.id] && symbol_table[token.id].lef_typ === "SUFFIX")
			return suffix(left, token, symbol_table[token.id].lef_rbp, symbol_table[token.id].lef_pat);
	...
	var suffix = function (lval, operator, rbp, pat) {
		advance();
		var result = find([], pat);
