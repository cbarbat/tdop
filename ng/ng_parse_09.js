...
var make_parse = function () {
	...
	var find = function (left, pat) {
		...
		if (pat.length > 1 && pat[0] === token.value) {
			t = token;
			advance();
			p = parse(pat[1], false); // pat[1] for rbp
	...
	// special symbols
	symbol("(end)");
	do_prefix("(start)", MIN_PREC + 1, ["LAMB"]);
	...
	// reserved words
	do_nilfix("true", 0, ["LAMB"]);
	do_nilfix("false", 0, ["LAMB"]);
	
	symbol(")");
	do_prefix("(", MIN_PREC + 1, [")"], function () { // parentheses for grouping
		var right, result;
		advance();
		right = parse(symbol_table["(start)"], false);
	...
	do_prefix("[", MIN_PREC + 1, ["]"], function () { // array definition
		...
		if (token.id !== "]") {
			advance();
			while (true) {
				a.push(parse(symbol_table["(start)"], false));
				if (token.id !== ",") {
					break;
				}
				advance(",");
	...
	do_suffix("!", 50, ["LAMB"]); // factorial

	do_infixR("^", 60, ["LAMB"]);
	
	do_prefix("+", 70, ["LAMB"]);
	do_prefix("-", 70, ["LAMB"], function () { // negation
		var p, result;
		advance();
		p = parse(this, false);
		result = {};
	...
	symbol(":");
	do_prefix("(:)", 200, ["LAMB"]);
	do_infixR("?", 200, [":", symbol_table["(:)"]]); symbol_table["?"].lef_rbp = MIN_PREC + 1;
	...
	return function (source) {
		...
		if (tokens) {
			...
			do {
				if (token.id !== "(end)") {
					result.push(parse(symbol_table["(start)"], false));
				} else {
					break;
				}
